...
interface EventItem {
  id: number;
  // Add other properties of event item here
  [key: string]: any;
}
...
function Event() {
  ...
  const fetchEvents = (page = 1, term = "") => {
    const query = `event?isEvent=true&LanguageId=${langId}&PageIndex=${page}&PageSize=${ITEMS_PER_PAGE}${
      term ? `&Search=${term}` : ""
    }`;
    setIsLoading(true);
        ...
        <div className="searchCard">
          ...
          <div className="inputSerch">
            <input
              ...
              placeholder="Search for the latest events that matter to you"
            />
          </div>
